    ...
    try {
      ...
      if (!result.canceled && result.assets.length > 0) {
        ...
        for (const asset of result.assets) {
          const uri = asset.uri;
          const fileName =
            asset.fileName || uri.split('/').pop() || `image_${Date.now()}`;
          const extension = fileName.split('.').pop()?.toLowerCase() || '';

          if (!allowedTypes.includes(extension)) {
            errors.push(
              `"${fileName}" is not allowed. Only JPG, JPEG, and PNG files are supported.`,
            );
            continue;
          }
          ...
          try {
            if (mode === 'create') {
              ...
              if (imageUri.startsWith('file://')) {
                // Local pending image
                const updatedPending = pendingImages.filter(
                  (img) => img !== imageUri,
                );
                ...
                const imageObj = imageObjects.find(
                  (obj) => obj.url === imageUri,
                );

                if (imageObj) {
                  const fileId = imageObj.id;
                  console.log(
                    `🗑️ Deleting backend image (id=${fileId}):`,
                    imageUri,
                  );
                  ...
                  if (success) {
                    ...
                    console.log(
                      '✅ Image deleted successfully and removed from state',
                    );
                  } else {
  ...
  const handleUpdateFiles = async () => {
    if (pendingImages.length === 0) {
      Alert.alert(
        ...
        'Please add images before updating.',
      );
    }
    setLoading(true);
      ...
      <View style={styles.modalOverlay}>
        <View style={[styles.modalContent, isDark && styles.modalContentDark]}>
          ...
          >
            ...
            >
              Add high-quality photos to showcase your property. The first image
              will be the main thumbnail.
              {'\n\n'}
              Maximum file size is 10MB. Supported file types: JPG, JPEG, and
              PNG.
            </Text>
            ...
            {images.length > 0 && (
              <>
                <View style={styles.imageCountContainer}>
                  <Text
                    style={[styles.imageCount, isDark && styles.imageCountDark]}
                  >
