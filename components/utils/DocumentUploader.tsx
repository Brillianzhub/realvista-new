...
const DocumentUploader: React.FC<DocumentUploaderProps> = ({
  ...
  const selectDocuments = async () => {
    try {
      ...
      const convertHeicToJpg = async (asset: any) => {
        ...
        result.assets
          ...
          .map(async (asset) => {
            ...
            if (['heic', 'heif'].includes(extension)) {
              const converted = await convertHeicToJpg(asset);
              if (converted) return converted;
            }

            // 🔥 STEP 2: normale Files
  ...
  const uploadDocuments = async () => {
    ...
    if (!selectedPropertyId) {
      Alert.alert(
        ...
        'Please select a property to upload documents to.',
      );
      return;
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={[styles.infoText, isDark && styles.infoTextDark]}>
        Maximum file size is 10MB. Ensure files are named accordingly, as these
        names will be used to list the files. This applies to documents and not
        necessarily to images. Supported file types: PDF, JPG, JPEG, PNG, HEIC,
        HEIF, MP3, and MP4.
      </Text>
