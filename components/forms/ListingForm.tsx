...
export default function ListingForm({
  ...
  const [formData, setFormData] = useState({
    ...
    listing_purpose:
      initialData?.listing_purpose ||
      initialData?.market_type.toLowerCase() ||
      '',
    ...
    lot_size:
      initialData?.lot_size?.toString() || initialData?._size?.toString() || '',
  ...
  const validateFormData = (data: typeof formData) => {
    ...
    if (
      !data.price ||
      isNaN(Number(removeCommas(data.price))) ||
      Number(removeCommas(data.price)) <= 0
    ) {
    ...
    if (data.property_type !== 'land') {
      ...
      if (
        !data.square_feet ||
        isNaN(Number(data.square_feet)) ||
        Number(data.square_feet) <= 0
      ) {
    ...
    if (
      !data.lot_size ||
      isNaN(Number(data.lot_size)) ||
      Number(data.lot_size) <= 0
    ) {
    ...
    if (data.property_type !== 'land' && data.year_built) {
      if (
        !/^\d{4}$/.test(data.year_built) ||
        Number(data.year_built) > new Date().getFullYear()
      ) {
  ...
  const handleSubmit = () => {
    ...
    const submissionData = {
      ...
      bedrooms:
        formData.property_type !== 'land' && formData.bedrooms
          ? Number(formData.bedrooms)
          : null,
      bathrooms:
        formData.property_type !== 'land' && formData.bathrooms
          ? Number(formData.bathrooms)
          : null,
      square_feet:
        formData.property_type !== 'land' && formData.square_feet
          ? Number(formData.square_feet)
          : null,
      lot_size: Number(formData.lot_size),
      year_built:
        formData.property_type !== 'land' && formData.year_built
      ...
      <View style={styles.radioGroup}>
        {listingPurposeChoices.map((item) => (
          <TouchableOpacity
            ...
            style={[
              styles.radioButton,
              formData.listing_purpose === item.value &&
